...
class EncoderBlock(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm=True, kernel_size=(3, 3, 1), padding_size=(1, 1, 0),
                 init_stride=(1, 1, 1)):
...
class YNet(nn.Module):
    ...
    def __init__(self, in_channel=1, n_class=3):
        super().__init__()

        features = [16, 32, 64, 128, 256, 512]

        self.encoder1_1 = EncoderBlock(self.in_channels, features[0])
        self.maxpool1_1 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.encoder1_2 = EncoderBlock(features[0], features[1])
        self.maxpool1_2 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.encoder1_3 = EncoderBlock(features[1], features[2])
        self.maxpool1_3 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.encoder1_4 = EncoderBlock(features[3], features[3])
        self.maxpool1_4 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.encoder2_1 = EncoderBlock(self.in_channels, features[0])
        self.maxpool2_1 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.encoder2_2 = EncoderBlock(features[0], features[1])
        self.maxpool2_2 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.encoder2_3 = EncoderBlock(features[1], features[2])
        self.maxpool2_3 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.encoder2_4 = EncoderBlock(features[3], features[3])
        self.maxpool2_4 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.bottleneck = EncoderBlock(features[4], features[5])

        self.upconv4 = nn.ConvTranspose3d(features[5], features[4], kernel_size=3, stride=1, padding=1)
        self.down4 = EncoderBlock(features[4] * 2, features[4])

        self.upconv3 = nn.ConvTranspose3d(features[4], features[3], kernel_size=3, stride=1, padding=1)
        self.down3 = EncoderBlock((features[2] * 6)*2, features[3])

        # ToDo Adding skip connetion beetween two encoders check here https://github.com/azadef/ynet/blob/master/networks.py#L183
